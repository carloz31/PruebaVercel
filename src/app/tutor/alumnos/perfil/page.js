...
const fetchAlumnoData = async (id) => {
  const alumnoResponse = await axios.get(`${process.env.backend}/alumnoApi/buscarAlumno/${id}`);
  const documentosResponse = await axios.get(`${process.env.backend}/planDeAccionApi/listarPlanesAccionPorIdAlumno/${id}`);
  
  const documentos = documentosResponse.data.map((documento) => ({
    key: documento.id,
    id: documento.id,
  ...
  return { alumno: alumnoResponse.data, documentos };
};

function AlumnoPerfilComponent({ id, user }) {
  ...
  useEffect(() => {
    const fetchAlumno = async () => {
      setIsLoading(true);
      try {
        const { alumno, documentos } = await fetchAlumnoData(id);
        setAlumno(alumno);
  ...
  return (
    <LayoutComponent siderItems={tutorItems} showFooter={false}>
      ...
      <Card title={<Title level={3} style={{ textAlign: "left" }}> {alumno.nombre} {alumno.apellidoPaterno} {alumno.apellidoPaterno} </Title>} style={{ width: "100%", fontFamily: 'Nunito, sans-serif', color: 'black' }}>
...
export default function AlumnoPerfil() {
  ...
  return (
    <Suspense fallback={<div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "100vh" }}><Spin size="large" /></div>}>
      <AlumnoPerfilComponent id={id} user={user} />
    </Suspense>
  );
}