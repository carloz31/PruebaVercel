...
const TableComponent = ({ isLoading, tipoTutorias, flagtable }) => {
  ...
  const handleModificar = async () => {
    try {
      ...
      const response = await axios.put(
        `/tipoTutoriaApi/actualizarTipoTutoria`,
        {
  ...
  const handleUpdate = (record) => {
    ...
    form.setFieldsValue({
      ...
      tipotutor: record.tipoTutorFijo,
      tipo: record.tipoTutor,
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      const response = await axios.post(
        `/tipoTutoriaApi/borrar_tipo_tutoria/${id}`,
      );
  ...
  return (
    <div
      style={{ overflowX: "auto", backgroundColor: "#fff", padding: "16px" }}
    >
